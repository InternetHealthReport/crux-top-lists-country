...
class CrUXDownloader:
    ...
    GLOBAL_SQL = """SELECT distinct origin, experimental.popularity.rank
        ...
        WHERE yyyymm = @date AND experimental.popularity.rank <= 1000000
    ...
    def dump_month_to_csv(self, scope, yyyymm: int, path):
        assert scope == "global" or scope in countries
        query = None
        job_config = None
        ...
        if scope == "global":
            ...
            job_config = bigquery.QueryJobConfig(
                query_parameters=[
                    bigquery.ScalarQueryParameter("date", "INT64", yyyymm),
                ]
            )
        else:
            ...
            job_config = bigquery.QueryJobConfig(
                query_parameters=[
                    bigquery.ScalarQueryParameter("date", "INT64", yyyymm),
                    bigquery.ScalarQueryParameter("country_code", "STRING", scope),
                ]
            )
...
class CrUXRepoManager:
    ...
    def _make_directories(self):
        ...
        for country in countries:
            dir = os.path.join(self._country_directory, country.alpha2.lower())
    ...
    def download(self, credentials_path=None, credentials_json=None, credentials_env=False):
        ...
        for country in countries:
            scope = country.alpha2.lower()
            data_directory = self._global_directory if scope == "global" else os.path.join(self._country_directory, scope)
